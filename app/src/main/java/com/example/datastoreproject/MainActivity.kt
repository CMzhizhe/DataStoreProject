...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        this.findViewById<Button>(R.id.user_sharefr_migrate_data_store_insert_int).setOnClickListener {
            DataStoreUtil.getInstance().put(SHARE_KEY_USER,KEY_SHARE_AGE,Random().nextInt())
        }

        //迁移完userInfo，往里面插入一条String类型的值
        this.findViewById<Button>(R.id.user_sharefr_migrate_data_store_insert_string).setOnClickListener {
            DataStoreUtil.getInstance().put(SHARE_KEY_USER,KEY_SHARE_NAME,"新插入的用户名字-${Random().nextInt()}")
        }
